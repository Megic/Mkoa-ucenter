...
module.exports = function ($this) {
    ...
    main['add'] = function *() {
        ...
        if (check.status) {/*通过验证*/
            ...
            if (where.id) {/*存在数据ID更新改数据*/
                res = yield $D('memberExtend').update($this.POST, {where: where});
                resData = $this.POST;
            } else {/*新增*/
                res = yield $D('memberExtend').build($this.POST).save();
                resData = res;
            }
    ...
    main['findOne'] = function *() {
        ...
        var res = yield $D('memberExtend').findOne({where: where}, {raw: true});
        $this.success(res);
    };
    ...
    main['findAll'] = function *() {
        ...
        var where = {};
        var res = yield $D('memberExtend').findAndCountAll({
        where: where,
        limit: perPages,
    ...
    main['delete'] = function *() {
        ...
        if (where.id)yield $D('memberExtend').destroy({where: where});
        $this.success();
    };
