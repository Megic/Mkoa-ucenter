...
module.exports = function ($this) {
    ...
    main['add'] = function *() {
        /*验证规则*/
        var rules = {
                openid: {rule:'required',error:'验证失败!'},
                mid: {rule:'required',error:'验证失败!'},
                type: {rule:'required',error:'验证失败!'}};
        ...
        if (check.status) {/*通过验证*/
            ...
            if (where.id) {/*存在数据ID更新改数据*/
                res = yield $D('memberOpen').update($this.POST, {where: where});
                resData = $this.POST;
            } else {/*新增*/
                res = yield $D('memberOpen').build($this.POST).save();
                resData = res;
            }
    ...
    main['findOne'] = function *() {
        ...
        var res = yield $D('memberOpen').findOne({where: where}, {raw: true});
        $this.success(res);
    };
    ...
    main['findAll'] = function *() {
        ...
        var where = {};
        var res = yield $D('memberOpen').findAndCountAll({
        where: where,
        limit: perPages,
    ...
    main['delete'] = function *() {
        ...
        if (where.id)yield $D('memberOpen').destroy({where: where});
        $this.success();
    };
