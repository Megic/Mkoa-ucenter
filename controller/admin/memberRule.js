...
module.exports = function ($this) {
    ...
    main['add'] = function *() {
        /*验证规则*/
        var rules = {
                title: {rule:'required',error:'验证失败!'},
                name: {rule:'required',error:'验证失败!'},
                status: {rule:'required',error:'验证失败!'}};
        ...
        if (check.status) {/*通过验证*/
            ...
            if (where.id) {/*存在数据ID更新改数据*/
                res = yield $D('memberRule').update($this.POST, {where: where});
                resData = $this.POST;
            } else {/*新增*/
                res = yield $D('memberRule').build($this.POST).save();
                resData = res;
            }
    ...
    main['findOne'] = function *() {
        ...
        var res = yield $D('memberRule').findOne({where: where}, {raw: true});
        $this.success(res);
    };
    ...
    main['findAll'] = function *() {
        ...
        var where = {};
        var res = yield $D('memberRule').findAndCountAll({
        where: where,
        limit: perPages,
    ...
    main['delete'] = function *() {
        ...
        if (where.id)yield $D('memberRule').destroy({where: where});
        $this.success();
    };
