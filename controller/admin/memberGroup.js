...
module.exports = function ($this) {
    ...
    main['add'] = function *() {
        /*验证规则*/
        var rules = {
                name: {rule:'required|max:40',error:'验证失败!'},
                rules: {rule:'',error:''},
                status: {rule:'required',error:''}};
        ...
        if (check.status) {/*通过验证*/
            ...
            if (where.id) {/*存在数据ID更新改数据*/
                res = yield $D('memberGroup').update($this.POST, {where: where});
                resData = $this.POST;
            } else {/*新增*/
                res = yield $D('memberGroup').build($this.POST).save();
                resData = res;
            }
    ...
    main['findOne'] = function *() {
        ...
        var res = yield $D('memberGroup').findOne({where: where}, {raw: true});
        $this.success(res);
    };
    ...
    main['findAll'] = function *() {
        ...
        var where = {};
        var res = yield $D('memberGroup').findAndCountAll({
        where: where,
        limit: perPages,
    ...
    main['delete'] = function *() {
        ...
        if (where.id)yield $D('memberGroup').destroy({where: where});
        $this.success();
    };
